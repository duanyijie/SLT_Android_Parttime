package com.leelistview.view;

import com.leelistview.util.DensityUtil;
...
public class LeeListView extends RelativeLayout implements OnScrollListener{
	...
	// -- header view
	private LeeListViewHeader mHeaderView;
	...
	// -- footer view
	private LeeListViewFooter mFooterView;
	 ...
	 * @param context
	 */
	public LeeListView(Context context) {
		super(context);
		initWithContext(context);
	}

	public LeeListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initWithContext(context);
	}

	public LeeListView(Context context, AttributeSet attrs, int defStyle) {
	...
	private void initWithContext(Context context) {
		...
		mListView = (ListView) LayoutInflater.from(context).inflate(R.layout.leelistview, null);
				...
				default:
					...
					if (mListView.getFirstVisiblePosition() == 0) {
						...
						if (mEnablePullRefresh
							...
							mHeaderView.setState(LeeListViewHeader.STATE_REFRESHING);
		...
		// init header view
		mHeaderView = new LeeListViewHeader(context);
		...
		// init footer view
		mFooterView = new LeeListViewFooter(context);
		...
		infoHint = (TextView) LayoutInflater.from(context).inflate(R.layout.leelistview_infohint, null);
	...
	public void setPullLoadEnable(boolean enable,String str) {
		...
		if (!mEnablePullLoad) {
			...
			mPullLoading = false;
			mFooterView.show();
			mFooterView.setState(LeeListViewFooter.STATE_NORMAL);
	...
	private void updateHeaderHeight(float delta) {
		...
		if (mEnablePullRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
			if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
				mHeaderView.setState(LeeListViewHeader.STATE_READY);
			} else {
				mHeaderView.setState(LeeListViewHeader.STATE_NORMAL);
			}
		}
	...
	private void updateFooterHeight(float delta) {
		...
		if (mEnablePullLoad && !mPullLoading) {
			if (height > PULL_LOAD_MORE_DELTA) { // height enough to invoke load
													// more.
				mFooterView.setState(LeeListViewFooter.STATE_READY);
			} else {
				mFooterView.setState(LeeListViewFooter.STATE_NORMAL);
			}
		}
	...
	private void startLoadMore() {
		mPullLoading = true;
		mFooterView.setState(LeeListViewFooter.STATE_LOADING);
		if (mListViewListener != null) {
